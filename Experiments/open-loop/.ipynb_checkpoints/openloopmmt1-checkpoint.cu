...
float *h_A, *h_B, *h_C;           // page-locked host arrays
float *dev_A, *dev_B, *dev_C;     // device aliases via cudaHostGetDevicePointer
int N = 512;
int jobs = 1;
...
void kernellaunch(dim3 gridDim, dim3 blockDim, int size) {
    ...
    for (int i = 0; i < jobs; ++i) {
        ...
        cudaEventCreate(&sp);
        cudaEventRecord(st);
        matMulKernel<<<gridDim, blockDim>>>(dev_A, dev_B, dev_C, N);
        cudaEventRecord(sp);
...
void runService(float setpoint, float period, float termination) {
    ...
    // device pointers that alias the same physical pages
    cudaHostGetDevicePointer((void**)&dev_A, h_A, 0);
    cudaHostGetDevicePointer((void**)&dev_B, h_B, 0);
    cudaHostGetDevicePointer((void**)&dev_C, h_C, 0);
